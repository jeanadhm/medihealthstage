...
function PatientRdv() {
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        ...
        setNotification({
          open: true,
          message: "Impossible de charger les docteurs.",
          severity: "error",
        });
      }
    };
  ...
  const handleSubmit = async () => {
    ...
    if (!patientId || !accessToken) {
      setNotification({
        open: true,
        message: "Erreur : patient ou token non identifié.",
        severity: "error",
      });
      return;
    }
    ...
    try {
      ...
      setNotification({
        open: true,
        message: error.message || "Échec de l'envoi de la demande.",
        severity: "error",
      });
    }
  };
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      <Box sx={{ maxWidth: 500, margin: "auto", padding: 4, mt: 10 }}>
